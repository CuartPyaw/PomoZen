...
// 获取计时器状态
function getTimerState(): Promise<TimerState> {
  return new Promise((resolve) => {
...
// 获取下一个模式
function getNextMode(currentMode: string, pomodoroCycle: number): string {
...
// 获取模式时长
function getModeDuration(mode: string, settings: Settings): number {
  switch (mode) {
    ...
    default: return settings.customFocusTime;
  }
}
...
chrome.alarms.onAlarm.addListener(async (alarm) => {
  ...
  const notif = notifications[state.mode as keyof typeof notifications];
  if (notif) {
  ...
  if (settings.autoSwitch) {
    ...
    const newState: TimerState = {
      mode: nextMode as 'focus' | 'break' | 'longBreak',
    ...
    switch (type) {
      ...
      case 'RESET': {
        ...
        const state: TimerState = {
          mode: (data?.mode || 'focus') as 'focus' | 'break' | 'longBreak',
          timeLeft: duration,
          isRunning: false,
          pomodoroCycle: 0,
        };
      ...
      case 'SET_MODE': {
        ...
        const newState: TimerState = {
          mode: data.mode as 'focus' | 'break' | 'longBreak',
          timeLeft: duration,
          isRunning: false,
